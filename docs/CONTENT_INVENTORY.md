...
- [x] **외국인등록증(체류카드) 수수료** (2026-08-30) — 법무부 고시와 출입국관리법 시행규칙 별표에서 발급·재발급 수수료 **35,000원**을 확인했다. 공식 자료에는 HiKorea/창구 경로별 법정 수수료 차이가 없어 단일 값만 남겼다.
- [x] **체류지 변경신고 기한** (2026-08-30) — 출입국관리법 제36조 기준 전입일부터 **15일 이내**, 수수료 없음, 정부24·관할 출입국·시군구 경로로 확인했다. 앱은 이 값을 임의 계산 기한으로 확대하지 않는다.
- [x] **HiKorea 방문예약 범위** (2026-08-30) — 공식 예약 안내에서 당일 예약 불가·가장 빠른 방문일은 다음 날임을 확인했다. 고정 슬롯 공개 주기는 공시되지 않아 앱의 희소성 주장을 제거하고 실제 가용성을 HiKorea에서 확인하도록 했다.
...
- [x] **NHIS 1577-1000 외국어 서비스** (2026-08-30) — NHIS 영문 Contact Us에서 현재 내선이 **6번**임을 확인해 코드·출국 안내를 수정했다. 외국어 직통번호 `033-811-2000`, 평일 09:00–18:00도 함께 안내한다.
- [x] **통신 3사 해지 절차** (2026-08-30) — SKT·KT·LG U+ 공식 페이지를 연결했다. SKT Easy의 채널, KT 공식 영문 페이지 간 충돌, LG U+ 공식 취소/고객센터 경로를 각각 표시하며 통신 3사 공통 절차를 단정하지 않는다. 해외·대리 해지와 요금제별 서류는 각 통신사가 최종 권위다.
- [x] **9개 대학 × 기숙사 일정의 상수 제거** (2026-08-30) — 2026년 공식 공지에서도 대학·건물·학기·중도퇴사 여부별 날짜와 반환 시점이 서로 달라 고정값이 될 수 없음을 재확인했다. 출국 미션의 1–2주/4–6주 주장을 제거하고 현재 배정 생활관 공지와 사무실을 최종 권위로 유지한다.
...
부가 필드: `mapHint` 11건, `titleKo` 10건, `appliesTo` (dormitory/off-campus 분기) 일부.
...
- [x] **`mapHint` 11건** (2026-08-30) — 실제 장소와 공식 접근 정보를 대조했다. 광장시장·인사동·서울역·경복궁·국립중앙박물관처럼 안정적인 단일 POI만 `mapSearchQuery`로 지도 행동을 제공한다. 편의점·공원·산·쇼핑지처럼 복수/일반 지시문은 설명으로만 남기며 불안정한 지도 검색을 만들지 않는다. 경복궁 접근은 공식 5번 출구로 수정했다.
- [x] **공휴일·계절 의존 미션** (2026-08-30) — 축제, 한강 치맥, 등산에 `seasonal` 메타데이터와 연 단위 검토 문구를 붙였고 테스트로 누락을 막았다. 실제 행사일·날씨는 상수로 저장하지 않는다.
...
- [x] 기숙사 신청 일정은 상수로 수집하지 않음 (`volatility: high`) — 9건 모두 현재 학기 공지를 직접 확인하도록 설계. 공지 자체의 매 학기 운영 갱신은 외부 콘텐츠 운영 항목이다.
...
- [x] **`visaTypeOrStatus` 현행성** (2026-08-30) — 법무부 체류외국인 자격별 통계표에서 D-2-6(교환학생)과 D-2-8(방문학생)이 모두 현행임을 확인했고 온보딩 영문 라벨도 바로잡았다.
...
| `crane` | Crane (학) | 장수 | 느린 여행, 템플스테이 |
| `lotus` | Lotus (연꽃) | 정화, 재생 | 자기돌봄, 기록, 학습 |
| `chaekgeori` | Chaekgeori (책거리/책가도) | 독서, 학문, 수양 | 책, 수업, 언어 |
| `sansuhwa` | Sansuhwa (산수화) | 자연 애호, 관조, 이상 경관 | 여행, 하이킹, 자연 |

- [x] **민화 상징 해설의 문화적 정확성 자료 검토** (2026-08-30) — 국립민속박물관·국립중앙박물관 자료로 6개 설명을 대조했다. 근거가 부족한 `학=평화`, `책거리=호기심`, `산수화=방랑` 단정을 앱 노출 문구에서 제거했다. 정식 출시 전 사람 문화 감수는 별도 승인 게이트다.
...
- [ ] **시대별 모티프 고증** — `DESIGN.md` / `BYEONGPUNG_ART_DIRECTION.md` 소관. 자료 검토로 책거리가 늦어도 18세기 후반 조선에서 확립된 장르임을 확인했고, 신라·고려 패널의 경전·경판 재해석을 아트 디렉션 문서에 명시했다. 다만 24폭 전체의 시대별 도상을 사람 문화 전문가가 최종 감수해야 이 게이트를 닫는다.
...
| 법무부 수수료 고시·법령정보센터 | `immigration.go.kr/bbs/immigration/47/590299/artclView.do`, `law.go.kr` 시행규칙 별표 | 체류카드 발급·재발급 35,000원 | 2026-08-30 |
| HiKorea 방문예약 | `hikorea.go.kr/resv/ResvIntroR.pt` | immigration-appointment, 당일 예약 불가·다음 날부터 선택 | 2026-08-30 |
| 정부24·출입국관리법 | `gov.kr` 체류지 변경 민원, `law.go.kr` 제36조 | 전입 후 15일·수수료 없음 | 2026-08-30 |
| SKT·KT·LG U+ 공식 지원 | `tworld.co.kr`, `globalshop.kt.com`, `lguplus.com` | G2 통신 해지의 통신사별 경로·충돌 | 2026-08-30 |
| 국민건강보험공단 영문 Contact Us | `nhis.or.kr/english/wbheaa02000m01.do` | G5·건강보험 외국어 내선 6 | 2026-08-30 |
...
| 국립중앙박물관 모란 큐레이터 추천 | `museum.go.kr/MUSEUM/contents/M0501000000.do?relicRecommendId=962059&schM=view` | 모란의 부귀·풍요 상징 | 2026-08-30 |
| 국립민속박물관 호랑이 전시 | `nfm.go.kr/user/planexhibition/home/20/selectPlanExhibitionLView.do?planExhibitionIdx=851` | 호랑이의 용맹·수호 의미 | 2026-08-30 |
| 국립민속박물관 소장품 전시 | `nfm.go.kr/user/planexhibition/home/62/selectPlanExhibitionLView.do?planExhibitionIdx=1151` | 학의 장수 의미 | 2026-08-30 |
| 국립중앙박물관 연꽃 특별전 | `museum.go.kr/MUSEUM/contents/M0202030000.do?exhiSpThemId=4112&schM=view` | 연꽃의 정화·재생 의미 | 2026-08-30 |
| 국립중앙박물관 책거리 큐레이터 추천 | `museum.go.kr/MUSEUM/contents/M0501000000.do?relicRecommendId=562147&schM=view` | 독서·학문·수양의 의미 | 2026-08-30 |
| 국립중앙박물관 산수화 전시 | `museum.go.kr/MUSEUM/contents/M0701010000.do?arcId=6738&catId=128&schM=view` | 자연 애호·관조·이상 경관 | 2026-08-30 |
| 국립민속박물관 책거리 해설 | `nfm.go.kr/home/1426/userVideoChannelDetail.do?no=605` | 18세기 후반 책거리의 확산 | 2026-08-30 |
| 국립전주박물관 책가도 전시 | `jeonju.museum.go.kr/specialBefore.es?act=view&mid=a10201020000&seq=1368` | 조선 후기 책거리/책가도 연대 | 2026-08-30 |
| 우주항공청 공고 제2026-0078호 | `kasa.go.kr/bbs/BBSMSTR_000000000018/B000000003234Li6nD2.do?mno=sub01_03_03` | 2027년도 월력요항·전국 공휴일 | 2026-08-30 |
...
`src/lib/holidays.ts`에 공식 2026·2027 월력요항 기반 공휴일·대체공휴일·선거일 테이블을 추가했다. 2026년 법 개정으로 공휴일이 된 노동절과 제헌절도 반영했다. 토·일요일과 공휴일을 구분하고, 테이블에 없는 연도는 `unknown`으로 반환해 날짜를 열려 있다고 추정하지 않는다.
...
구현 진입점: `governmentDayStatus()`, `adjustDueDateToPreviousGovernmentBusinessDay()`, `addGovernmentBusinessDays()`. 2027년 목록은 우주항공청 공고 제2026-0078호를 기준으로 2026-08-30에 확인했다. 임시공휴일·뜻밖의 선거일은 사후 지정될 수 있으므로 최신 정부 공고와 방문 기관을 최종 권위로 유지한다. 2028년 이후는 공식 월력요항을 확인한 뒤 테이블을 갱신해야 한다.
...
**완료 (2026-08-30):** 여섯 템플릿에 각각 5개의 `suggestedItems`를
추가했다. 새 목록 화면은 선택하지 않은 제안 칩으로 보여 주며 사용자가
탭한 항목만 저장한다. E2E가 선택 1건·직접 새로고침을 검증한다.
...
| 단계 | 내용 | 전제 |
|---|---|---|
| **N0** | ③ 관할 기관 매핑, ④ 공휴일 테이블 | **2026-08-30 갱신.** 9개 대학 캠퍼스 구를 관할표에 연결했고, 교외 거주는 실제 등록 구 입력 전 `review_required`다. 2026·2027년 공휴일은 공식 표를 반영했으며 2028년 이후는 `unknown`이다. 체크리스트에 표시 완료 |
...
| **N2** | ⑩ `completeWhen` + `evidence` 필드, ⑪ Want-to 시드 | **2026-08-30 완료.** 55개 원장·카드별 출처 UI와 여섯 템플릿의 선택형 시드 5개씩을 반영. 특정 담당자와 학기별 일정 갱신은 후속 운영 작업 |
