...
**executor는 이 절을 판정하지 않는다.** step 6과 같은 항목을 **step 8 종료 시 한 번 더** 잰다 —
**step 6 이후 step 7·8이 콘텐츠와 테스트를 건드리므로 회귀를 볼 자리가 필요하다.**

- [ ] **콘솔 error 0건** (검증 **③-b**)
- [ ] **접근성 `TC-121`·`TC-122`** — 스크린리더가 **차단 2종을 구분해 읽고 차단 사유 배너를 낭독한다** (검증 **⑤**)

> ⛔ **「각 step에서 잰다」로 적지 않는다.** 브라우저 레인은 **step 6·8 두 번만** 돈다 —
> 실행 경로가 없는 조건을 「각 step」으로 적어 둔 것이 `47` §2.1 **★10**이 잡은 결함이다.
> **executor 몫(③-a 번들 성립)은 8개 step 전부**, **브라우저 레인 몫(③-b·⑤)은 이 두 곳**이다.

## ⛔ 이 step이 하지 않는 것

- **`TC` 156건 전건 자동화** — 남은 시간에 맞춰 **슬라이스에 걸리는 것부터** 한다.
  (🔻 2026-07-27 — `TC-160`이 원인 2종으로 갈려 **155 → 156**이다. `44` §2.1 ★1)
