...
$ rg '^\| ADR-[0-9]{4} \|' .work/adr-dec-raw-v4.md |
...
| ADR-0007 | Cold-start splash handler ref | 불명 | DEC-001, DEC-024 | “**Chosen:** A React ref (`coldStartHandledRef`) in `AuthGate` (or root `_layout`). On first mount it sets itself true and forces a one-time redirect through `/splash`, after which the restored route resumes.” (`docs/adr/0007-cold-start-splash-handler-ref.md:28`) | “**로그인을 두지 않는다.**” (`DEC-001`, 28:39) / `MEM-02` Byeongpung은 **이번 범위 밖 — 구현하지 않는다** (`DEC-024`, 31:468,477). 병풍·`AuthGate`와 무관하게 cold-start splash가 필요한지는 신 설계에 근거가 없다. **무엇을 더 봐야 판정할 수 있는지:** 독립적인 cold-start splash 요구사항, 새 root boot/navigation 흐름, splash의 현재 책임을 확인해야 한다. | `app/_layout.tsx`, `app/(onboarding)/splash.tsx` — 필요성 확정 전에는 구현 기준으로 고정하지 않음 |
...
| ADR-0010 | Housing-specific mission tagging (`appliesTo`) | 뒤집힘 | DEC-024 | “**Chosen:** `appliesTo?: 'dormitory' \| 'off-campus'` field. Missions without the field are universal (applies to both).” (`docs/adr/0010-housing-applies-to-tagging.md:28`) | `MEM-01 Cultural missions` — **없음 — `Won't`(범위 밖)** `DEC-024` (`.work/pmjob/k-journey/19-k-journey-f03-ia-screen-inventory-2026-07-25.md:192`) / “**이번 범위 밖 — 구현하지 않는다**” (`31-k-journey-decision-log-2026-07-25.md:477`) | `src/data/missions.ts`, `app/(tabs)/index.tsx`, `src/lib/firebase.ts` — 50건 문화 미션 카탈로그의 `appliesTo` 필터 전제를 이번 구현 근거에서 격리; 행정 서류 규칙은 이 대조표의 대상이 아님 |
...
\* `DEC-027`은 현재 로그에 기록되어 있으나 `44-k-journey-role-review-unscored-dec-2026-07-27.md`에 의해 확정되지 않은 상태다. `DEC-026`은 삭제분(`save_pending`·`sync_conflict`, `E2`~`E6`)이 **확정(유지)**이고, 신설분(`save_failed`·`E8`)만 확정되지 않은 상태다. 위 표는 기록된 DEC가 요구하는 방향으로 대조했으며, 확정 여부와 구현 격리는 §7에 별도로 남겼다.
...
- **무엇이 무효가 되는가:** `pending writes`, 원격 값과 로컬 값의 충돌, 재연결 후 `Synced.`, 다른 기기에서 갱신된 bucket count를 사용자에게 보여주는 원격 sync visual·상태 전제다. `DEC-026`은 이 중 삭제분을 확정(유지)했다.
...
- **신 DEC이 요구하는 방향:** **삭제 — 확정(유지):** `save_pending`·`sync_conflict` 상태와 `E2`~`E6` 전이는 걷어낸다. **이번 구현에 적용한다.** **신설 — 미확정:** 로컬 저장 실패 `save_failed` 상태와 `E8` 전이는 별도 세션 확정 전까지 구현 기준으로 고정하지 않고 격리한다.
- **되돌릴 조건:** `DEC-001` 또는 `DEC-022`가 되돌아가 서버가 사용자 값을 갖게 되면 삭제한 원격 동기화 상태·전이를 새로 도출한다. `DEC-026`은 삭제한 ID를 `E9`부터 다시 붙이라고 했다. 익명 이벤트 큐 유실을 측정할 수 있고 유실률이 5%를 넘는 경우도 재검토 조건이다.
...
- **무엇이 무효가 되는가:** `appliesTo?: 'dormitory' | 'off-campus'` 두 값 union과 누락 시 양쪽에 적용하는 규칙을 **50 curated missions across 4 phases × 4 categories** 카탈로그의 필터로 쓰는 전제다. `MEM-01 Cultural missions`이 `DEC-024`에서 `Won't`(범위 밖)이므로 이 문화 미션 카탈로그 필터는 이번 구현의 근거에서 격리한다. 이것은 행정 서류 규칙이 아니다.
- **그 ADR을 근거로 짜인 코드·문서:** `src/data/missions.ts`, `app/(tabs)/index.tsx`, `src/lib/firebase.ts`, ADR의 `missionsForHousing` 전제. 원문은 “Some Have-To missions only make sense for one population”과 홈의 “X of 50 done”을 설명한다 (`docs/adr/0010-housing-applies-to-tagging.md:10-12`); 실제 카탈로그와 `MissionCategory`도 `src/data/missions.ts:1-13`에 있다.
- **신 DEC이 요구하는 방향:** `DEC-024`의 `MEM-01 Cultural missions` `Won't`를 적용해 `appliesTo` 기반 문화 미션 필터와 그 50건 분모를 이번 구현의 근거에서 격리한다. `DEC-003`·`DEC-018`의 신 행정 서류 모델은 별개 사안이며, 이 대조표가 그 모델을 결정하지 않는다.
...
- **되돌릴 조건:** 신호는 누적 필요조건이 아니다. `DEC-024` 필드 10의 각 신호 — **진입 비율 10% 초과**, 카드 소팅 **`I6`**에서 `Memory`를 실용 흐름과 묶는 결과, **8월 말 코호트 인터뷰**에서 문화 콘텐츠 요구 — 는 해당 신호를 관측하면 **`DEC-024`의 Memory 범위를 재검토**한다. `DEC-001`의 세 신호 — 완료 이력 소실이 **중대 불만 상위 3위**, **기기 교체가 체류 중 흔하다는 근거**, **`MET-006` 보조 지표(내보내기 실행률) 저조** — 는 각각 **`DEC-001`의 대안 A를 재검토**한다. 각 경우 **신호 관측 → 해당 결정 재검토 → 새 결정 확정**의 3단을 거치며, 신호 하나도 기존 ADR이나 sign out·Memory 범위를 자동 부활시키지 않는다. `DEC-024` 신호가 여는 것은 Memory 범위이고, sign out 관련 범위는 `DEC-001`의 해당 재검토와 새 결정 없이는 되살아나지 않는다.

### ADR-0034 — Photo upload pipeline

- **무엇이 무효가 되는가:** `users/{uid}/photos/{missionId}/{ulid}.jpg` Firebase Storage 업로드와 문화 미션 사진을 갤러리·병풍에 연결하는 네 부분 파이프라인을 이번 구현 전제로 두는 것이다. `DEC-001`·`DEC-022`의 로컬 데이터 원칙과 `MEM-01`~`MEM-03`의 `Won't`가 동시에 충돌한다.
- **그 ADR을 근거로 짜인 코드·문서:** `DESIGN.md §20`, `docs/ANALYTICS_SCHEMA.md`의 `photo_upload_outcome`, `docs/EMPTY_STATES.md §§5-7`, `docs/PLAY_DATA_SAFETY.md`의 사진 수집 표.
  - **ADR이 지목한 target(현재 부재):** `src/lib/photoUpload.ts`, `src/components/photo/PhotoCaptureButton.tsx`, `src/components/photo/PhotoUploadProgress.tsx`, `storage.rules` — ADR 자체가 “**no photo upload code today**”라고 적는다 (`docs/adr/0034-photo-upload-pipeline.md:171,173-186`).
  - **실재하는 인접 코드:** `src/lib/share.ts`는 “**Capture-and-share helpers for the byeongpung gallery**”이며 `captureRef` + `Sharing.shareAsync`를 사용한다 (`src/lib/share.ts:1-6`). 사진 업로드 코드는 아니다.
- **신 DEC이 요구하는 방향:** 사용자 조건·태스크 상태는 로컬에만 두고, UID 기반 Storage 업로드와 문화 미션 사진의 갤러리·병풍 연결은 이번 구현의 근거에서 제외한다. 다만 병풍 이미지를 OS share sheet로 내보내는 로컬 `src/lib/share.ts` 범위는 유지한다. `docs/PLAY_DATA_SAFETY.md`의 “Photos / files: Not collected”와도 맞춰, ADR 파일은 남기되 구현은 격리한다.
- **되돌릴 조건:** 신호는 누적 필요조건이 아니다. `DEC-024` 필드 10의 각 신호 — **진입 비율 10% 초과**, 카드 소팅 **`I6`**에서 `Memory`를 실용 흐름과 묶는 결과, **8월 말 코호트 인터뷰**에서 문화 콘텐츠 요구 — 는 관측되면 **`DEC-024`의 Memory 범위를 재검토**한다. `DEC-022` 필드 10의 여섯 신호도 각각 본다: ① **‘콘텐츠 갱신’ 사유 해소율 20% 미만** → 푸시 범위 재검토, ② **OS 알림 개수 상한이 필요한 기한 알림 수보다 작음** → 푸시 범위 재검토, ③ **`DEC-001` 되돌림** → `DEC-001` 재검토, ④ **감사 로그에 사용자 식별 값 유입** → 감사 로그 범위 재검토, ⑤ **편집자 1인 지속** → 역할 분리 재검토, ⑥ **OTA로 분류한 변경이 새 빌드를 요구한 사례** → OTA 판정 재검토. 각 신호는 **신호 관측 → 해당 결정 재검토 → 새 결정 확정**의 3단이고, 하나도 자동 부활 조건이 아니다. 이 중 UID 사진 업로드에 직접 걸리는 것은 ③뿐이다. ③을 관측해도 사진 서버 저장이나 ADR-0034가 자동으로 되살아나지 않으며, `DEC-001` 재검토 뒤 사진 서버 저장을 허용하는 새 결정이 확정되어야 한다.
...
| `DESIGN.md` §21 ↔ DEC-026 | **참 — 직접 충돌한다.** **삭제분 적용 = 확정. §21의 원격 sync visual은 걷어낸다.** | §21은 `Synced.`, `Updated from another device.`, “**eventual consistency**”와 다른 기기 conflict를 시각 계약으로 둔다(`DESIGN.md:697-713`). DEC-026은 `save_pending`·`sync_conflict`와 `E2`~`E6` 삭제를 확정(유지)하고, `save_failed`·`E8` 신설은 미확정으로 둔다. §21.1의 offline dot 자체는 남을 수 있지만 재연결·원격 conflict 문장은 살려 둘 수 없다. |
...
- **ADR-0010(주거 미션 태깅):** `src/data/missions.ts`, `app/(tabs)/index.tsx` — `DEC-024`의 `MEM-01 Cultural missions` `Won't` 범위로 `appliesTo` 기반 50건 미션 카탈로그 필터를 격리한다. 이는 행정 서류 규칙이 아니며, `DEC-003`·`DEC-018`의 신 모델은 이 대조표가 결정하지 않는다.
...
- **ADR-0034(사진 업로드):** `DESIGN.md §20`, `docs/ANALYTICS_SCHEMA.md` `photo_upload_outcome`, `docs/EMPTY_STATES.md` §§5-7, `docs/PLAY_DATA_SAFETY.md`.
- **DEC-024(병풍 `Won't`) 영향:** `src/lib/share.ts` — “**Capture-and-share helpers for the byeongpung gallery**” (`src/lib/share.ts:1-6`). 로컬 OS share sheet 범위는 ADR-0034의 업로드 제거와 별개로 유지한다.
...
- `docs/ERROR_MESSAGES.md`: **충돌 확인.** `auth-*`·`network-offline-recovered`·`bucket-conflict` 등 **삭제분에 걸린 행은 걷어낸다(확정)**. `save_failed`·`E8`의 새 문구는 **미확정이므로 이번에 추가하지 않는다**.
...
2. **DEC-026 확정성:** `DEC-026`은 ADR-0031의 기존 원격 sync 상태·전이 **삭제를 확정(유지)**했지만, `save_failed`·`E8` 신설은 미확정이다. 같은 세션에서 작성·채점되어 V5가 닫히지 않았으므로, §7에 적은 대로 `save_pending`·`sync_conflict` 및 `E2`~`E6` 삭제는 적용하고 `save_failed`·`E8`만 구현 기준에서 격리한다.
...
아래 표는 `뒤집힘`·`보강` 중 미확정 DEC-026·DEC-027에 직접 걸린 항목만 분리한 것이다. DEC-026 내부의 확정된 삭제와 미확정인 신설을 나눠 적고, 확정 의존과 미확정 의존을 한 구현 표로 섞지 않는다.
...
그 밖의 범위 변경은 이 표의 미확정 DEC-026·DEC-027 의존으로 분류하지 않는다. 특히 인증·원격 사용자 데이터·Memory 범위는 확정된 `DEC-001`·`DEC-022`·`DEC-024`에 따라 구현 여부를 판단한다. 그 밖의 보강은 §1에 적은 확정 DEC의 결정 축을 따른다. ADR-0010의 범위 격리는 `DEC-024`의 `MEM-01 Cultural missions` `Won't`에 따른다. `DEC-003`·`DEC-018`의 신 행정 서류 모델은 별개 사안이며 이 대조표가 결정하지 않는다.
...
$ rg -c '^\| ADR-[0-9]{4} \|' .work/adr-dec-raw-v4.md
35
...
최종 확인 결과는 `ADR 35 = §1 표 행 35 = 판정 분포 합계 35`다. 기존
`.work/adr-dec-raw.md`, `.work/adr-dec-raw-v2.md`, `.work/adr-dec-review-v2.md`, 코드,
`docs/`, `CLAUDE.md`는 수정하지 않았다.
