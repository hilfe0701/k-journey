...
$ rg '^\| ADR-[0-9]{4} \|' .work/adr-dec-raw-v3.md |
...
- **되돌릴 조건:** `DEC-024` 필드 10의 **진입 비율 10% 초과**, 카드 소팅 **`I6`**에서 `Memory`를 실용 흐름과 묶는 결과, **8월 말 코호트 인터뷰**에서 문화 콘텐츠 요구가 나오는 경우를 먼저 본다. 여기에 `DEC-001`의 되돌릴 조건인 **완료 이력 소실이 중대 불만 상위 3위**, **기기 교체가 체류 중 흔하다는 근거**, **`MET-006` 보조 지표(내보내기 실행률) 저조**도 함께 필요하다. `DEC-024`의 되돌림은 Memory 범위만 다시 열며, sign out 관련 범위는 이 `DEC-001` 조건 없이는 되살아나지 않는다.
...
- **되돌릴 조건:** `DEC-024` 필드 10의 **진입 비율 10% 초과**, 카드 소팅 **`I6`**에서 `Memory`를 실용 흐름과 묶는 결과, **8월 말 코호트 인터뷰**에서 문화 콘텐츠 요구가 나오는 경우를 본다. 다만 이것만으로 `users/{uid}` 서버 업로드가 되살아나지는 않는다. `DEC-022`의 서버 범위 되돌릴 조건인 **‘콘텐츠 갱신’ 사유 해소율 20% 미만**, **OS 알림 개수 상한이 필요한 기한 알림 수보다 작음**, **`DEC-001` 되돌림**, **감사 로그에 사용자 식별 값 유입**, **편집자 1인 지속**, **OTA로 분류한 변경이 새 빌드를 요구한 사례**도 함께 확인해야 한다.
...
$ rg -c '^\| ADR-[0-9]{4} \|' .work/adr-dec-raw-v3.md
35
...
`.work/adr-dec-raw-v1.md`, `.work/adr-dec-raw-v2.md`, `.work/adr-dec-review-v2.md`, 코드,
